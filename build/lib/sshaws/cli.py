...
class SSHAWSClient:
    ...
    @staticmethod
    def _strip_session_banners(output: bytes) -> str:
        """Remove SSM session banner lines from command output."""
        if not output:
            return ''
        ...
        filtered = [
            ...
            if not line.startswith('Starting session with SessionId:')
            and not line.startswith('Exiting session with sessionId:')
        ]
    ...
    def run_ssm_command(
        ...
        if cleaned_out:
            sys.stdout.write(cleaned_out)
            sys.stdout.flush()

        cleaned_err = self._strip_session_banners(stderr_bytes)
        if cleaned_err:
            sys.stderr.write(cleaned_err)
            sys.stderr.flush()
